...
python3 -m py_compile bin/obsws.py bin/audioctl.py bin/safetyctl.py bin/collabctl.py tests/test_obsws.py tests/test_audioctl.py tests/test_safetyctl.py tests/test_collabctl.py
...
required = (
    ...
    "collab.copy-invite", "collab.copy-program",
)
...
python3 -m unittest -v tests/test_obsws.py tests/test_audioctl.py tests/test_safetyctl.py tests/test_collabctl.py
...
if python3 bin/collabctl.py status | grep -E '"(room|password|inviteUrl|programUrl|directorUrl)"'; then
  echo "collaboration status exposes room credentials" >&2
  exit 1
fi
